...
const EncryptionProcess = () => {
  ...
  const finishedRef = useRef(false); // Guard against double-execution (React StrictMode)
  ...
  useEffect(() => {
    ...
    const processFile = async () => {
      ...
      const finish = async () => {
        // Guard: only run once even if React StrictMode re-invokes
        ...
        try {
          ...
          const r = await fetch(state.fileData);
          const blob = await r.blob();
          const file = new File([blob], state.fileName || "file", { type: blob.type });

          const isDecrypting = state.mode === "decrypt";
          const options = { mode: state.mode || "encrypt" };
          
          if (state.brainwaveData) {
            options.brainwaveData = state.brainwaveData;
            console.log("Attached brainwaveData to cryptographic options layer");
          }
          ...
          saveToHistory(state.fileName, state.mode || 'encrypt', 0, 'failed');
          enc.setIsProcessing(false);
          alert.error(`Encryption failed: ${error.message}`);
          setTimeout(() => navigate('/encryption', { replace: true }), 3000);
        }
      };

      // small delay to let UI render nicely
      starterTimeout = setTimeout(() => runStep(0), 300);
    };

    processFile();

    // Return cleanup to useEffect so React StrictMode cancels correctly
    return () => {
