...
const EncryptionResult = () => {
  ...
  const handleDownload = () => {
    try {
      ...
      // For decryption, use the filename from backend (already has "decrypted_" prefix)
      // For encryption, add "encrypted_" prefix
      const filename = isDecryption
        ? (enc.uploadedFileName ?? "decrypted_file")
        : `encrypted_${enc.uploadedFileName ?? "file"}`;
      a.download = filename;
  ...
  const handleUploadToIpfs = async () => {
    ...
    try {
      ...
      fd.append("file", new File([blob], `encrypted_${enc.uploadedFileName || "file"}`, { type: "application/octet-stream" }));
      fd.append("original_filename", enc.uploadedFileName || "file");

      const res = await fetch(`/ipfs/upload`, { method: "POST", body: fd });
  ...
  const handleShare = async () => {
    try {
      const target = window.prompt("Enter target IP (machine running backend on port 5006)");
      if (!target) return;
      const apiBase = "";
      // Fetch the encrypted blob from the in-memory URL
      const blob = await (await fetch(enc.resultUrl)).blob();
  ...
  const layerCards = [
    { name: isDecryption ? "Brainwave Reversal" : "Brainwave Layer", emoji: "🧠", color: "from-purple-500/20 to-pink-500/20" },
    { name: isDecryption ? "PQC Decryption" : "PQC Layer", emoji: "🧩", color: "from-cyan-500/20 to-blue-500/20" },
    { name: isDecryption ? "Time Restoration" : "Time Dilation", emoji: "⏳", color: "from-green-500/20 to-emerald-500/20" },
  ];
      ...
      >
        ...
        >
          {layerCards.map((layer, idx) => (
            <motion.div
              key={layer.name}
              className={`bg-gradient-to-br ${layer.color} backdrop-blur-sm border border-gray-700/40 rounded-lg p-4 text-center`}
            ...
            >
              ...
              <div className="text-sm font-medium text-gray-100 mb-1">{layer.name}</div>
        ...
        >
          ...
          <motion.button
            onClick={() => {
              ...
              enc.setUploadedFileName(null);
              enc.setIsProcessing(false);
              enc.setProgress(0);
              // Navigate back
              navigate(isDecryption ? "/encryption/decrypt" : "/encryption");
            }}
