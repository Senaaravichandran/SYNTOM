...
import BioAmpMonitor from "../components/BioAmpMonitor";
...
const EncryptionDecryption = () => {
  ...
  const handleFetchFromIpfs = async () => {
    ...
    try {
      const response = await fetch(`/ipfs/fetch/${ipfsCid.trim()}`);
      if (!response.ok) {
        // Try to get detailed error from backend
        let errorMsg = "Failed to fetch file from IPFS";
        try {
  ...
  const handleStart = async () => {
    ...
    if (!brainwaveData) return alert.error("Please capture BioAmp EXG Neural signal first.");
    ...
    reader.onload = () => {
      ...
      navigate("/encryption/process", {
        state: { fileName: file.name, fileData: dataUrl, mode: "decrypt", brainwaveData: brainwaveData },
      });
    };
    reader.onerror = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden" style={{ backgroundColor: 'transparent' }}>
      ...
      <div className="w-full max-w-6xl px-6 py-12 relative z-10">
        ...
        {/* BioAmp EXG Neural Interface */}
        <div className="max-w-2xl mx-auto">
          <BioAmpMonitor onCaptureComplete={setBrainwaveData} />
        </div>
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="flex items-center gap-3 w-full justify-center">
                <motion.button
                  ...
                  whileHover={(file && brainwaveData) ? { scale: 1.02 } : {}}
                  whileTap={(file && brainwaveData) ? { scale: 0.98 } : {}}
                >
                  <Unlock size={20} />
