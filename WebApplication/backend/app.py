...
# Load environment variables from a root .env file and standard locations
root_env = Path(__file__).parent.parent.parent / ".env"
backend_env = Path(__file__).parent / ".env"
ipfs_env = Path(__file__).parent.parent.parent / "IPFS_Integration" / ".env"
if root_env.exists(): load_dotenv(root_env)
if backend_env.exists(): load_dotenv(backend_env)
if ipfs_env.exists(): load_dotenv(ipfs_env)

# Add parent directory to path for feature imports FIRST, before any other imports
parent_path = str(Path(__file__).parent.parent.parent)
sys.path.insert(0, parent_path)
print(f"DEBUG: Added to sys.path: {parent_path}")
print(f"DEBUG: Post_Quantum_Cryptography exists: {Path(parent_path, 'Post_Quantum_Cryptography').exists()}")
...
# Note: uhae_hqc module is not required; hybrid_crypto provides all needed functionality
HAS_FULL_CRYPTO = True

# Hybrid encryption from Post_Quantum_Cryptography
try:
    # Verify module exists before importing
    pqc_path = Path(parent_path, 'Post_Quantum_Cryptography')
    if not pqc_path.exists():
        raise ImportError(f"Post_Quantum_Cryptography module not found at {pqc_path}")
    
    from Post_Quantum_Cryptography.hybrid_crypto import encrypt_file_to_package, decrypt_package_to_plaintext, extract_package_id  # type: ignore
    from Post_Quantum_Cryptography.crypto_core import secure_random_bytes, load_entanglement_seeds, generate_entanglement_seed_csv, generate_quantum_entropy  # type: ignore
    from Time_Security.time_dilation import compute_chunk_delay  # type: ignore
    print("SUCCESS: Post_Quantum_Cryptography modules loaded successfully")
except ImportError as e:
    print(f"WARNING: Post_Quantum_Cryptography modules not available: {e}")
    import traceback
    traceback.print_exc()
...
try:
    from Braided_Key_Exchange.path_establishment import establish_path_locally, SessionConfig  # type: ignore
    from Braided_Key_Exchange.braided_key_derivation import derive_session_key  # type: ignore
    from Braided_Key_Exchange.secure_channel import create_secure_sender, create_secure_receiver, SecureMessage  # type: ignore
...
try:
    from IPFS_Integration.auto_uploader import get_uploader  # type: ignore
    from IPFS_Integration.pinata_service import PinataService  # type: ignore
...
def braided_derive_key():
    ...
    try:
        ...
        if session_json_path.exists():
            ...
            from Braided_Key_Exchange.path_establishment import BraidOrder  # type: ignore
        ...
        from Braided_Key_Exchange.braided_key_derivation import BraidedKeyDeriver  # type: ignore
    ...
    try:
        ...
        if options_str:
            try:
                ...
                brainwave_data = opts.get("brainwaveData")
            except Exception:
                pass

        # True Real Flow Injection!
        if brainwave_data is not None:
            print("=== INITIATING FULL REAL QKD FLOW ===")
            print("1. Extracting Neural Key using Fuzzy Extractor & SNN...")
            from Eeg_Brainwaves.neural_key_gen import generate_brainwave_key
            from Quantum_Key_Distribution.topological_braiding import TopologicalBraider
            
            raw_eeg_key = generate_brainwave_key(brainwave_data)
            
            print("2. Topological Anyonic Braiding with Time Dilation (IBM Quantum QKD)...")
            braider = TopologicalBraider(raw_eeg_key, time_dilation_offset=0.15)
            braided_k_root = braider.generate_braided_key()
            
            # Inject our real flow keys into the request
            eeg_key_hex = raw_eeg_key.hex()
            k_root_hex = braided_k_root.hex()
        ...
        if use_braided:
            try:
                ...
                _req_k_root = request.form.get("k_root")
                if k_root_hex is not None:
                    # from our True Real Flow
                    k_root = bytes.fromhex(k_root_hex)
                    print(f"   [KEY] Using K_root from True Real Flow QKD!")
                elif _req_k_root:
                    print(f"   [KEY] Using K_root from frontend (from Temporal page)...")
                    k_root = bytes.fromhex(_req_k_root)
                else:
        ...
        try:
            from Post_Quantum_Cryptography.crypto_core import HQC_KEM, Dilithium_Sign  # type: ignore
            from Post_Quantum_Cryptography.hybrid_crypto import encrypt_file_to_package, extract_package_id  # type: ignore
            ...
            print(f"Hybrid crypto failed: {e}. Falling back to demo XOR.")
...
def decrypt_file():
    ...
    try:
        ...
        if brainwave_data is not None:
            print("=== INITIATING FULL REAL QKD FLOW DECRYPTION ===")
            print("1. Reversing Neural Key Extraction (Fuzzy Extractor & SNN)...")
            from Eeg_Brainwaves.neural_key_gen import generate_brainwave_key
            from Quantum_Key_Distribution.topological_braiding import TopologicalBraider
            ...
            eeg_key_hex = raw_eeg_key.hex()
            # K_root comes from the session in decrypt
        ...
        print(f"   session_id: {session_id}")
        print(f"   use_braided: {use_braided}")
        print(f"   brainwave_data present: {brainwave_data is not None}")
        ...
        if use_braided:
            try:
                ...
                if not k_root:
                    print(f"   No stored K_root, deriving new one (may fail if time offset was used)")
        ...
        try:
            from Post_Quantum_Cryptography.hybrid_crypto import decrypt_package_to_plaintext, extract_package_id  # type: ignore
            ...
            if magic_bytes != expected_magic:
                print(f"   Not a BWC encrypted file, magic mismatch")
            ...
            if keys_path.exists():
                ...
                print(f"   Keys file not found, generating fresh keypair (may fail)")
                # Fallback: generate fresh keypair (demo only; will fail signature or unwrap)
                from Post_Quantum_Cryptography.crypto_core import HQC_KEM  # type: ignore
                hqc = HQC_KEM()
            ...
            try:
                ...
                if original_filename == "file" and file.filename:
                    ...
                    if uploaded_name.startswith('encrypted_'):
                        ...
                        print(f"   Stripped 'encrypted_' prefix: {uploaded_name}")
                    elif uploaded_name.startswith('encrypted-'):
                        uploaded_name = uploaded_name[10:]
                        print(f"   Stripped 'encrypted-' prefix: {uploaded_name}")
...
def test_ipfs_connection():
    ...
    try:
        from IPFS_Integration.pinata_service import get_pinata_service  # type: ignore
...
def fetch_from_ipfs(cid):
    ...
    try:
        ...
        try:
            uploader = get_uploader()
            pinata_service = uploader.pinata
            ...
            print(f"   Could not retrieve filename from Pinata metadata: {e}")
            ...
            try:
                ...
                print(f"   Could not retrieve filename from logs: {e2}")
        
        # Fetch file content from IPFS
        response = None
        last_error = None
        
        # Strategy 1: Use Pinata dedicated gateway (auto-discover)
        try:
            uploader = get_uploader()
            pinata_jwt = uploader.pinata.jwt_token if hasattr(uploader.pinata, 'jwt_token') else os.getenv('PINATA_JWT_TOKEN')
            if pinata_jwt:
                auth_headers = {"Authorization": f"Bearer {pinata_jwt}"}
                
                # Verify pin exists first
                print(f"   Checking Pinata for CID...")
                pin_check = requests.get(
                    f"https://api.pinata.cloud/data/pinList?hashContains={cid}&status=pinned",
                    headers=auth_headers, timeout=15, verify=False
                )
                
                if pin_check.status_code == 200 and pin_check.json().get('rows'):
                    print(f"   CID found on Pinata!")
                    
                    # Auto-discover dedicated gateway
                    dedicated_gw = None
                    try:
                        gw_response = requests.get(
                            "https://api.pinata.cloud/v3/ipfs/gateways",
                            headers=auth_headers, timeout=10, verify=False
                        )
                        if gw_response.status_code == 200:
                            gw_data = gw_response.json()
                            gateways_list = gw_data.get('data', {}).get('rows', [])
                            if gateways_list:
                                domain = gateways_list[0].get('domain')
                                if domain:
                                    dedicated_gw = f"https://{domain}/ipfs/{cid}"
                                    print(f"   Found dedicated gateway: {domain}")
                    except Exception as gw_err:
                        print(f"   Could not auto-discover gateway: {gw_err}")
                    
                    # Try dedicated gateway if found
                    if dedicated_gw:
                        try:
                            print(f"   Trying dedicated gateway: {dedicated_gw[:60]}...")
                            r = requests.get(dedicated_gw, timeout=60, verify=False)
                            if r.status_code == 200 and len(r.content) > 0:
                                response = r
                                print(f"   SUCCESS: Got {len(r.content)} bytes from dedicated gateway")
                            else:
                                last_error = f"Dedicated gateway HTTP {r.status_code}"
                                print(f"   Dedicated gateway returned {r.status_code}")
                        except Exception as e:
                            last_error = str(e)
                            print(f"   Dedicated gateway error: {e}")
                    
                    # Try Pinata conversion endpoint as backup
                    if response is None:
                        try:
                            convert_url = f"https://api.pinata.cloud/data/pinList?hashContains={cid}"
                            print(f"   Trying Pinata direct access...")
                            # Try with subdomain-style gateway
                            subdomain_url = f"https://{cid}.ipfs.dweb.link"
                            r = requests.get(subdomain_url, timeout=30, verify=False)
                            if r.status_code == 200 and len(r.content) > 0:
                                response = r
                                print(f"   SUCCESS: Got {len(r.content)} bytes via subdomain gateway")
                            else:
                                last_error = f"Subdomain gateway HTTP {r.status_code}"
                                print(f"   Subdomain gateway returned {r.status_code}")
                        except Exception as e:
                            last_error = str(e)
                            print(f"   Subdomain gateway error: {e}")
                else:
                    print(f"   CID not found on Pinata, trying public gateways...")
                    last_error = "CID not found on Pinata"
        except Exception as e:
            last_error = str(e)
            print(f"   Pinata API error: {e}")
        
        # Strategy 2: Try public IPFS gateways as fallback
        if response is None:
            gateways = [
                ...
                f"https://flk-ipfs.xyz/ipfs/{cid}",
            ]
            
            for gw_url in gateways:
                try:
                    print(f"   Trying gateway: {gw_url[:60]}...")
                    r = requests.get(gw_url, timeout=30, verify=False)
                    if r.status_code == 200 and len(r.content) > 0:
                        response = r
                        print(f"   SUCCESS: Got {len(r.content)} bytes from {gw_url[:40]}")
                        break
                    else:
                        last_error = f"HTTP {r.status_code}"
                        print(f"   Gateway returned {r.status_code}, trying next...")
                except Exception as gw_err:
                    last_error = str(gw_err)
                    print(f"   Gateway error: {gw_err}, trying next...")
        
        if response is None:
            return jsonify({
                "status": "error",
                "message": f"Failed to fetch from IPFS. Last error: {last_error}"
            }), 502
        ...
        print(f"   Encrypted filename: {filename}")
        print(f"   Original filename: {original_name}")
        
        headers = {
            "Content-Disposition": f"attachment; filename={filename}",
            "X-Original-Filename": filename,
            "X-Decrypted-Filename": original_name,
            "X-IPFS-Hash": cid
        }
